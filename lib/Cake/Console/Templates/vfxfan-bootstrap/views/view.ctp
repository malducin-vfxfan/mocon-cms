	...
	foreach ($associations as $type => $data) {
		foreach ($data as $alias => $details) {
			if ($details['controller'] != $this->name && !in_array($details['controller'], $done)) {
				echo "\t\t\t<li><?php echo \$this->Html->link('List " . Inflector::humanize($details['controller']) . "', array('controller' => '{$details['controller']}', 'action' => 'admin_ndex'), array('class' => 'btn')); ?> </li>\n";
...
<div class="row">
	<aside class="admin-related">
		<h3><?php echo "\t\t<?php echo 'Related " . Inflector::humanize($details['controller']) . "';?>";?></h3>
...
<div class="row">
	<aside class="admin-related">
		<h3><?php echo "<?php echo 'Related " . $otherPluralHumanName . "';?>";?></h3>
		<?php echo "\t\t<?php if (!empty(\${$singularVar}['{$alias}'])):?>\n";?>
		...
		echo "\t\t\techo \$this->Paginator->first('last');\n";
		echo "\t\t?>\n";
		?>
		</div>
		<?php echo "<?php endif; ?>\n\n";?>
